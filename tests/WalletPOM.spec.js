...
test("Wallet screen should be visible", async ({ page }) => {
  ...
  await expect(page.locator(Wallet.WalletFullpage)).toBeVisible({
    timeout: 200000,
  });
});
...
test("Gift cards screen should be visible, even when empty", async ({
  page,
}) => {
  const login = new Login_page(page);
  const Home = new Home_page(page);
...
test.skip("Redeem gift if balance > ₹250 and button is visible", async ({
  page,
}) => {
  const login = new Login_page(page);
  const Home = new Home_page(page);
...
test("Verify Gift Cards Empty State Screen Display", async ({ page }) => {
  ...
  await page.locator(Wallet.MyGift_cards).click();
    await page.waitForTimeout(3000);


  if (await page.locator(Wallet.Redeemed_giftcards).isVisible()) {
  console.log("Redeemed giftcards section is visible");
} else if (await page.locator(Wallet.Giftcard_empty_state).isVisible()) {
  console.log("Giftcard empty state is visible");
}
});


test("Verify Coupons Cards Empty State Screen Display", async ({ page }) => {
  const login = new Login_page(page);
  const Home = new Home_page(page);
  const Wallet = new Wallet_page(page);
 ...
 if (await page.locator(Wallet.Redeemed_Coupons).isVisible()) {
  console.log("Coupon screen is visible");

} else if (await page.locator(Wallet.Giftcard_empty_state).isVisible()) {
  console.log("empty screen is visible");
}
